...
from mAP import precision_recall
...
def train_net(cfg):
    ...
    if cfg.pretrained!=None:# TODO test issue
        model.load_state_dict(torch.load(cfg.pretrained,map_location=lambda storage, loc: storage.cuda(rank)),strict=False)
...
def box_self_union(box):
    """box in shape (N,4)
    (N,4)->(N*N,4)->(N*(N-1),4)"""
    n=len(box)
    a=np.tile(box[:,np.newaxis,:],(1,n,1))
    b=np.tile(box[np.newaxis,:,:],(n,1,1))
    uarr=np.zeros_like(a)
    # box union
    uarr[:,:,0]=np.where(a[:,:,0]<b[:,:,0],a[:,:,0],b[:,:,0]) # x1
    uarr[:,:,1]=np.where(a[:,:,1]<b[:,:,1],a[:,:,1],b[:,:,1]) # y1
    uarr[:,:,2]=np.where(a[:,:,2]>b[:,:,2],a[:,:,2],b[:,:,2]) # x2
    uarr[:,:,3]=np.where(a[:,:,3]>b[:,:,3],a[:,:,3],b[:,:,3]) # y2
    return uarr[True^np.eye(n,dtype=bool)]
...
def test_func(data_loader, model, epoch, cfg):
    ...
    for samples in data_loader:
        ...
        box_num=samples['box_num_volumn'].to(rank)
        key_frame=samples['key_frame'].squeeze(-1)
        file_ids=samples['file_id_volumn']
        id_szs=samples['id_sz_volumn']
        bboxes=samples['bbox_volumn']
        ...
        for b,t in enumerate(key_frame):
            ...
            n=box_num[b,t]
            act_grd=action[b,t,:n].cpu().numpy()
            act_groundtruth.append(act_grd)
            inter_grd=interaction[b,t,:n*(n-1)].cpu().numpy()
            inter_groundtruth.append(inter_grd)
            act_scr=torch.softmax(act_score[ahead:ahead+n],dim=-1)
            # act_pred=torch.argmax(act_scr,dim=-1)
            act_conf,act_pred=torch.max(act_scr,dim=-1)
            act_predict.append(act_pred.cpu().numpy())
            inter_scr=torch.softmax(inter_score[ihead:ihead+n*(n-1)],dim=-1)
            # inter_pred=torch.argmax(inter_scr,dim=-1)
            inter_conf,inter_pred=torch.max(inter_scr,dim=-1)
            inter_predict.append(inter_pred.cpu().numpy())
            
            ahead+=n
            ihead+=n*(n-1)
            
            # prepare for mAP 
            file_id=file_ids[b,t].cpu().numpy()
            id_sz=id_szs[b,t]
            file_name=pickle.loads(file_id[:id_sz])
            
            bbox=bboxes[b,t,:n].cpu().numpy()*np.array([orig_w,orig_h,orig_w,orig_h])
            ibox=box_self_union(bbox)
            
            grd={'image':{'file_name':file_name},
                 'action_grd':act_grd.tolist(),
                 'bboxes':bbox.tolist(),
                 'inter_grd':inter_grd.tolist(),
                 'inter_bboxes':ibox.tolist()}
            
            pred={'image':{'file_name':file_name},
                  'action_pred':act_pred.tolist(),
                  'action_confidence':act_conf.tolist(),
                  'bboxes':bbox.tolist(),
                  'inter_pred':inter_pred.tolist(),
                  'inter_confidence':inter_conf.tolist(),
                  'inter_bboxes':ibox.tolist(),
                  }
    ...
    y_truth=gather_into_list(y_truth)
    y_pred=gather_into_list(y_pred)
    ...
    if is_master():
        ...
        if cfg.test: # save results
            y_truth_path=Path(__file__).parent/'mAP_output'
            y_truth_path.mkdir(parents=True,exist_ok=True)
            with open(y_truth_path/'y_truth.json','w') as f:
                json.dump(y_truth,f)
            with open(y_truth_path/'y_pred.json','w') as f:
                json.dump(y_pred,f)
        
        # mAP
        # mAP=precision_recall(y_truth,y_pred,show=cfg.test)
