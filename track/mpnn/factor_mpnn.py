...
class factor_mpnn(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.mapping_modules = torch.nn.ModuleList([
            iid_mapping(self.node_feature_dim, self.map_dim)
        ]) # 0 map node, 1: map factor
        # number of factor types
        ...
        self.mp_nn_modules = torch.nn.ModuleList([])
        self.mp_merge_modules = torch.nn.ModuleList([])
        self.final_filter = final_filter
        # construct feature mapping layers
        # each layer has 2 component map neural network, merge neural network
        for idx in range(len(dim_mapping_list) - 1):
            cmodule = torch.nn.ModuleList([])
